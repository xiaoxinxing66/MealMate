package com.sivan.feeds.service;
...
import com.sivan.feeds.mapper.FeedsMapper;
...
public class FeedsService {
    ...
    public List<FeedsVO> selectForPage(Integer page, String accessToken) {
        ...
        // 我关注的好友的 Feedkey  已经保存到Redis中了，这里我们直接拿到key
    ...
    private List<Integer> findFollowers(Integer dinerId) {
        ...
        List<Integer> followers = (List<Integer>) result.getData();
        return followers;
    }
    /**
     * 删除 Feed
    ...
    public void addFollowingFeed(Integer followingDinerId, String accessToken, int type) {
        ...
        if (type == 0) {
            // 取关
            // 拿到了我们取关的用户的所有feed id信息。
            List<Integer> feedIds = feedsList.stream()
                    .map(feed -> feed.getId())
                    .collect(Collectors.toList());
            // 移除 当前用户:key 以及 feed id的信息。
